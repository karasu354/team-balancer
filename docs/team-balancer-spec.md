...
| 3-8 | 手動分割モード       | 自動分割とは別に、手動で Blue/Red のロール配置を編集できる | ✅ 実装済み |
| 3-9 | DnD ロール割り当て   | プレイヤーカードをドラッグ&ドロップでロール枠へ配置する    | ✅ 実装済み |
...
| 5-13 | OPGG風テーマ           | ダーク基調 + ブルーアクセントのテーマ、カードレイアウト、ヘッダー導線を提供する                                    | ✅ 実装済み           |
| 5-14 | 入力プレビュー強化     | 単体入力でランクバッジとレートプレビューを表示し、入力時の判断コストを下げる                                       | ✅ 実装済み           |
| 5-15 | 分割比較可視化         | Blue/Red の総レート差とプレイヤー貢献度バーを表示し、結果の納得感を高める                                          | ✅ 実装済み           |
| 5-16 | キーボード導線         | 単体入力は Enter で追加、分割エリアは Enter で分割実行できる                                                       | ✅ 実装済み           |
| 5-17 | 入力可読性保証         | 白背景の入力要素では黒文字を強制し、テーマ差異があっても入力値を視認できるようにする                               | ✅ 実装済み           |
| 5-18 | 入力フォーム開閉       | プレイヤー追加フォームをアコーディオンで開閉できる                                                                 | ✅ 実装済み           |
| 5-19 | 分割/履歴タブ切替      | チーム分割領域で「分割結果」と「履歴」をタブ切り替えし、情報密度を制御する                                         | ✅ 実装済み           |
| 5-20 | 分割準備率の整理       | 分割準備率（%）表示を廃止し、分割条件の案内を分割エリア側へ集約する                                                | ✅ 実装済み           |
| 5-21 | サンプル投入表示制御   | サンプル投入ボタンはプレイヤー未登録時のみ表示し、登録後は非表示にする                                             | ✅ 実装済み           |
| 5-22 | サブタイトル整理       | ヘッダーの補助サブタイトルを削除し、主要導線を優先表示する                                                         | ✅ 実装済み           |
| 5-23 | コピー文言日本語化     | 分割結果コピー操作の文言を日本語（クリップボードにコピー）へ統一する                                               | ✅ 実装済み           |
| 5-24 | タブ切替時の高さ安定化 | 分割結果/履歴タブ切替時にコンテナ高さを揃え、下端スクロールでのレイアウトジャンプを抑制する                        | ✅ 実装済み           |
| 5-25 | 分割中の状態可視化     | チーム分け実行中は旧結果を隠し、新規計算中であることが分かる表示へ切り替える                                       | ✅ 実装済み           |
| 5-26 | 結果確定の再確認       | 試合結果確定前に再確認を行い、確定後は勝敗選択状態をクリアする                                                     | ✅ 実装済み           |
...
### UI方針（採用決定: 2026-05-04）

- OPGG 風 UI（ダーク背景、ブルーアクセント、カード型情報設計）を採用
- 入力・保存/復元・一覧・分割の主要導線をファーストビューで視認できる配置を採用
- API 契約は変更せず、表示層のみ刷新する
...
$$
score = 0.2 \times totalRatingDifference + 0.6 \times laneRatingDifference + 0.2 \times adcSupPairDifference
$$
...
- 2026-05-04 更新: 総レート差の寄与を下げるため、重みを `0.2 / 0.6 / 0.2` へ調整
