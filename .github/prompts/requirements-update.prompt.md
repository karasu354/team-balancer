---
agent: 'agent'
description: '要件修正。実装中・実装後のフィードバックにより既存 .steering の requirements.md を追記・修正する'
---

# 要件修正

既存の `.steering/[YYYYMMDD]-[連番]-[機能名]/requirements.md` を修正してください。
...
3. 対象フォルダの `requirements.md` を読み込んでから修正を行う
...
> **重要**: このプロンプトが触れてよいファイルは **対象 `.steering/` フォルダ内の `requirements.md` のみ**。
...
- やらないこと（たとえ要件変更の影響があっても、このプロンプトでは変更しない）:
  ...
  - `design.md` / `tasks.md` の変更（要件変更による影響を伝えることは可能）
  ...
  - その他 `.steering/[対象フォルダ]/requirements.md` 以外のすべてのファイル
